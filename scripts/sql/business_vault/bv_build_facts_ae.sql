...
CREATE OR REPLACE TABLE FACT_PROPERTY_KPI_AE AS (
  SELECT
    ...
    , CASE pk.CURRENCY_BASIS
        ...
        WHEN 1 THEN sd.SCENARIO_CURRENCY
        ELSE NULL
  ...
  FROM
    ...
    INNER JOIN DATA_VAULT.SAT_AE_SCENARIO_DETAILS           sd ON s.MD5_HUB_AE_SCENARIO = sd.MD5_HUB_AE_SCENARIO
);
...
CREATE OR REPLACE TABLE FACT_PROPERTY_PAYMENT_AE AS (
  SELECT
    ...
    , CASE pp.CURRENCY_BASIS
      ...
      END AS FK_DIMENSION_CURRENCY
    , pp.CURRENCY_BASIS
    , pp.IS_ASSURED_RESULTSET
    , pp.RESULTSET
    , pp.LINE_ITEM_TYPE_NAME AS LINE_ITEM_TYPE
    , pp.ACCOUNT_CODE -- TODO: Might be needed as part of the PK in future once we have data
...
CREATE OR REPLACE TABLE FACT_PROPERTY_CASHFLOW_AE AS (
  SELECT
    ...
    , CASE pc.CURRENCY_BASIS
      ...
      END AS FK_DIMENSION_CURRENCY    
    , pc.CURRENCY_BASIS
    , pc.IS_ASSURED_RESULTSET
    , pc.LINE_ITEM_TYPE_NAME AS LINE_ITEM_TYPE
    , pc.ACCOUNT_CODE -- TODO: Might be needed as part of the PK in future once we have data
...
CREATE OR REPLACE TABLE FACT_LEASE_CASHFLOW_AE AS (
  ...
  FROM
    ...
    -- Scenario --
    INNER JOIN DATA_VAULT.LINK_AE_PROPERTY_SCENARIO  s ON pp.MD5_HUB_AE_PROPERTY = s.MD5_HUB_AE_PROPERTY
    INNER JOIN DATA_VAULT.SAT_AE_SCENARIO_DETAILS   sd ON sd.MD5_HUB_AE_SCENARIO = s.MD5_HUB_AE_SCENARIO
);
...
CREATE OR REPLACE TABLE FACT_REVEX_PAYMENT_AE AS (
  SELECT
    ...
    , CASE ry.CURRENCY_BASIS
      ...
      END AS FK_DIMENSION_CURRENCY
    , ry.CURRENCY_BASIS
    , ry.IS_ASSURED_RESULTSET
    , ry.REVEX_TYPE_ENUM AS REVEX_TYPE
    , ry.RESULTSET
...
CREATE OR REPLACE TABLE FACT_REVEX_CASHFLOW_AE AS (
  SELECT
    ...
    , CASE rc.CURRENCY_BASIS
      ...
      END AS FK_DIMENSION_CURRENCY
    , rc.CURRENCY_BASIS
    , rc.IS_ASSURED_RESULTSET
    , rc.REVEX_TYPE_ENUM AS REVEX_TYPE
...
CREATE OR REPLACE TABLE FACT_LOAN_PAYMENT_AE AS (
  SELECT
    ...
    , CASE lp.CURRENCY_BASIS
      ...
      END AS FK_DIMENSION_CURRENCY
    , lp.CURRENCY_BASIS
    , lp.IS_ASSURED_RESULTSET
    , lp.RESULTSET
    , lp.LINE_ITEM_NAME AS LINE_ITEM_TYPE
    , lp.ACCOUNT_CODE
    , lp.AMOUNT
  FROM
    -- Loan --
